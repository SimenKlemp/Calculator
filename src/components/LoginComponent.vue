<template>
  <div id="loginContainer">
    ...
    <form class="login" @submit.prevent=SignIn>
      ...
      <label id="loginstatusLabel">{{ userInfo.loginStatus }}</label>
  </div>
  <div v-if = "userInfo.loginStatus === 'Fail'" id = registrationLink>
    <p>Not registred? Dont worry, just press this
...
export default {
  ...
  methods: {
    async SignIn() {
      let loginResponse = await loginUser(this.userInfo);

      if(loginResponse.loginStatus === "Success" ){
        this.$store.dispatch('postLogin', loginResponse)
        this.$router.push({
          name: 'Home'
        })
      }
        ...
        /*
      await this.$store.dispatch('postLogin', this.userInfo).then(() => {
